...
void Matrix<number>::matrixTransporn() {
    for (int r = 0; r < row; r++) {
        for (int c = 0; c < column; c++) {
            cout << matrix[c][r] << '\t';
        }
...
template <class number>
void Matrix<number>::setMatrixByArray(number** valueArray){
    for (int i = 0; i < row; i++) {
        delete[] matrix[i];
    }
    delete[] matrix;
...
template <class number>
int Matrix<number>::rank_by_gaussian_transformation() {
...
template <class number>
number* Matrix<number>::getRow(int i){
    return matrix[i];
}

template <class number>
bool Matrix<number>::setUserMatrix() {
